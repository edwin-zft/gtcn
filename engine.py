...
class trainer():
    ...
    def train(self, input, real_val):  # B, C , N, H
        self.model.train()
        self.optimizer.zero_grad()
        input = nn.functional.pad(input,(1,0,0,0))  # 左填充 时间维度上+1

        output = self.model(input) # 对输入数据进行训练 进行预测
        output = output.transpose(1,3)  #输出后还原原来的维度
        #output = [batch_size,12,num_nodes,1]
        real = torch.unsqueeze(real_val,dim=1)
        predict = self.scaler.inverse_transform(output)  # 将标准化的数据转为原始数据
