...
parser.add_argument('--data',type=str,default='data/METR-LA',help='data path')  # 输入数据地址
parser.add_argument('--adjdata',type=str,default='data/adj_mx.pkl',help='adj data path')  # 邻接矩阵的输入
...
def main():
    ...
    for i in range(1,args.epochs+1):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):  # 啊？？？？？？？？？？？？？？？？、、、
            trainx = torch.Tensor(x).to(device)
            trainx= trainx.transpose(1, 3)   # b 12 n c -> b c n 12
            trainy = torch.Tensor(y).to(device)
            trainy = trainy.transpose(1, 3)
            metrics = engine.train(trainx, trainy[:,0,:,:])  # 衡量指标 用一个y来对比x
